...
SELECT Sum(CASE
       ...
       Sum(CASE
             ...
             ELSE 0
           END) AS world_total_null_values,
       Sum(CASE
             WHEN m.languages IS NULL THEN 1
             ELSE 0
           END) AS language_total_null_values,
       Sum(CASE
             WHEN m.production_company IS NULL THEN 1
             ELSE 0
           END) AS production_total_null_values
FROM   movie AS m; 
...
/*
So from the output for the above queries it is evident that the year 2017 has produced highest number of movies and
the highest number of movies have been produced in the month of March
*/
...
SELECT Count(*) AS Total_number_of_movies_released
FROM   movie
...
SELECT   *,
         Rank() OVER (ORDER BY vote_count DESC) AS production_comp_rank
...
WITH top_actor
     AS (SELECT NAME
                ...
                Count(rm.movie_id)
                   AS movie_count,
                Cast(Sum(avg_rating * total_votes) / Sum(total_votes) AS
                     DECIMAL(8, 2))
                   AS
                actor_avg_rating,
                Rank()
...
-- the first cte fetches the list of the actresses who have acted in atleast 3 Indian movies
WITH actresses AS
(
         SELECT   n.id,
                  NAME,
...
-- the second cte fetches the list of top actresses who fulfill above condition and are now ranked based on hindi movies
top_actresses AS
(
         SELECT   NAME                                                                 AS actress_name,
                  Sum(total_votes)                                                     AS total_votes,
                  Count(rm.movie_id)                                                   AS movie_count,
                  Cast(Sum(avg_rating                 * total_votes)/Sum(total_votes) AS DECIMAL(8,2)) AS actress_avg_rating,
...
FROM     movie_duration window w1  AS (ORDER BY genre rows UNBOUNDED PRECEDING),
         w2                        AS (ORDER BY genre rows 12 PRECEDING);
...
-- the first cte gets the top three genres i.e. genres with max movie count
 WITH cte AS
(
         SELECT   genre,
                  Count(movie_id) AS movie_count
         ...
         GROUP BY genre
         ORDER BY movie_count DESC limit 3 ),
-- cte mid converts the income column to $ for standard comparison - here I have taken the conversion rate for INR to USD as 1/75
cte_mid AS
(
       SELECT id,
              CASE
                     WHEN LEFT(worlwide_gross_income,3) = 'INR' THEN Cast(Substring(worlwide_gross_income, 4, Length(worlwide_gross_income)) AS DECIMAL(20,2))/75.0
                     ELSE Cast(Substring(worlwide_gross_income, 2, Length(worlwide_gross_income)) AS                                            DECIMAL(20,2))
              END AS worlwide_gross_income
       FROM   movie
       WHERE  worlwide_gross_income IS NOT NULL ),
-- here we assign ranks to movies based on income for every year
cte_latest AS
(
         SELECT   cte.genre,
                  year,
                  ...
                  Dense_rank() OVER (partition BY year ORDER BY ctemid.worlwide_gross_income DESC) AS movie_rank
         FROM     movie m
         JOIN     genre g
         ON       g.movie_id = m.id
         JOIN     cte cte
         ON       cte.genre = g.genre
         JOIN     cte_mid ctemid
         ON       ctemid.id = m.id )
SELECT *
FROM   cte_latest
...
(
         SELECT   name_id AS director_id,
                  ...
                  date_published,
                  Lead(date_published, 1, date_published) OVER (partition BY NAME ORDER BY date_published, NAME) AS next_date_published
         FROM     director_mapping dm
         JOIN     names n
         ON       n.id = dm.name_id
         JOIN     movie m
